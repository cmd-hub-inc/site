...
export function FrameworkBadge({ fw }) {
  ...
  return (
    <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 700, background: c.bg, color: c.color }}>
      {fw}
    </span>
  )
}
...
export function TypeBadge({ type }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 700, background: 'rgba(255,255,255,0.06)', color: TYPE_COLORS[type] || C.muted }}>
      {icons[type]} {type}
    </span>
  )
}
